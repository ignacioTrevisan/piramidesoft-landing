...
const ProductModal: React.FC<ProductModalProps> = ({ isOpen, onClose, product, onSave }) => {
  ...
  const [tipos, setTipos] = useState([
    ...
    { id: "2", titulo: "Ventas" },
    { id: "3", titulo: "Facturación" },
    { id: "4", titulo: "CRM" },
    { id: "5", titulo: "E-commerce" },
    { id: "6", titulo: "App Móvil" }
  ]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...formData,
      precioAntes: formData.precioAntes ? parseFloat(formData.precioAntes) : null,
  ...
  const addSubtitulo = (moduloIndex: number) => {
    setFormData(prev => ({
      ...prev,
      modulos: prev.modulos.map((mod, i) => 
        i === moduloIndex ? { ...mod, subtitulos: [...mod.subtitulos, ""] } : mod
      )
    }));
  };
  ...
  const removeSubtitulo = (moduloIndex: number, subtituloIndex: number) => {
    setFormData(prev => ({
      ...
      modulos: prev.modulos.map((mod, i) => 
        i === moduloIndex 
          ? { ...mod, subtitulos: mod.subtitulos.filter((_, j) => j !== subtituloIndex) }
          : mod
      )
    }));
  };

  const updateSubtitulo = (moduloIndex: number, subtituloIndex: number, value: string) => {
    setFormData(prev => ({
      ...prev,
      modulos: prev.modulos.map((mod, i) => 
        i === moduloIndex 
          ? { 
              ...mod, 
              subtitulos: mod.subtitulos.map((sub, j) => j === subtituloIndex ? value : sub)
            }
          : mod
      )
    }));
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, tipoId: e.target.value }))}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                required
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, precioAntes: e.target.value }))}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, precioAhora: e.target.value }))}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                required
              />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, url_full: e.target.value }))}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
          </div>
          ...
          <div>
            <label className="flex items-center space-x-2">
              ...
              />
              <span className="text-sm font-medium text-gray-700">Producto visible</span>
            </label>
          </div>
          ...
          <div>
            ...
            {formData.modulos.map((modulo, moduloIndex) => (
              <div key={moduloIndex} className="border border-gray-200 rounded-lg p-4 mb-4">
                <div className="flex justify-between items-center mb-3">
                  <h4 className="font-medium text-gray-700">Módulo {moduloIndex + 1}</h4>
                  <button
                    type="button"
                ...
                <input
                  type="text"
                  value={modulo.titulo}
                  onChange={(e) => updateModulo(moduloIndex, "titulo", e.target.value)}
                  ...
                  {modulo.subtitulos.map((subtitulo, subtituloIndex) => (
                    <div key={subtituloIndex} className="flex space-x-2 mb-2">
                      <input
                        type="text"
                        value={subtitulo}
                        onChange={(e) => updateSubtitulo(moduloIndex, subtituloIndex, e.target.value)}
                        placeholder="Subtítulo"
                        className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                      />
                      <button
                        type="button"
                        onClick={() => removeSubtitulo(moduloIndex, subtituloIndex)}
                        className="px-2 py-1 bg-red-500 text-white rounded hover:bg-red-600"
      ...
      setProducts([
        {
          ...
          descripcion: "Control completo de inventario con reportes detallados y alertas automáticas.",
          precioAntes: 150000,
          precioAhora: 120000,
          imagenes: ["/stock-management.jpg"],
          video: "https://example.com/video1",
          url_demo: "https://demo.example.com",
          url_full: "https://app.example.com",
          visible: true,
          tipoId: "1",
          tipo: { id: "1", titulo: "Inventario" },
          modulos: [
            {
              id: "1",
              titulo: "Control de Stock",
              subtitulos: ["Inventario en tiempo real", "Alertas de stock mínimo"]
            }
          ],
            ...
            <div className="p-6">
              <div className="flex justify-between items-start mb-4">
                <div className="flex-1">
                  <h3 className="text-xl font-semibold text-gray-800 mb-2" style={{ fontSize: '18px' }}>
                    {product.titulo}
                  </h3>
                  <p className="text-sm text-gray-500 mb-2" style={{ fontSize: '12px' }}>
                    {product.tipo.titulo}
                  </p>
                  <p className="text-gray-600 text-sm mb-3 line-clamp-2" style={{ fontSize: '14px' }}>
                    {product.descripcion}
                  </p>
                </div>
